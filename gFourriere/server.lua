...
TriggerEvent('::{korioz#0110}::esx_society:registerSociety', 'fourriere', 'fourriere', 'society_fourriere', 'society_fourriere', 'society_fourriere', {type = 'public'})
...
AddEventHandler('gfourriere:getStockItem', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_fourriere', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
				...
				TriggerClientEvent('::{korioz#0110}::esx:showNotification', _source, 'Objet retiré', count, inventoryItem.label)
		else
...
AddEventHandler('gfourriere:ouvert', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('::{korioz#0110}::esx:showAdvancedNotification', xPlayers[i], 'Fourrière', '~g~Annonce', 'La fourrière est désormais ouverte', 'CHAR_FOURRIERE', 8)
	end
end)
...
AddEventHandler('gfourriere:ferme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('::{korioz#0110}::esx:showAdvancedNotification', xPlayers[i], 'Fourrière', '~r~Annonce', 'La fourrière est désormais fermé à plus tard!', 'CHAR_FOURRIERE', 8)
	end
end)