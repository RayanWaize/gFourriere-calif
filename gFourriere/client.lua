                                ...
                                if (GetOnscreenKeyboardResult()) then
                                    ...
                                    if result then
                                        ...
                                        if player ~= -1 and distance <= 3.0 then
                                            TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(player), 'society_fourriere', ('Fourrière'), montant)
                                            TriggerEvent('::{korioz#0110}::esx:showAdvancedNotification', 'Fl~g~ee~s~ca ~g~Bank', 'Facture envoyée : ', 'Vous avez envoyé une facture d\'un montant de : ~g~'..montant.. '$ ~s~pour cette raison : ~b~' ..raison.. '', 'CHAR_BANK_FLEECA', 9)
                                        else
                    ...
                    RageUI.ButtonWithStyle("Retirer argent de société",nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            if amount == nil then
                                ...
                                TriggerServerEvent('::{korioz#0110}::esx_society:withdrawMoney', 'fourriere', amount)
                    ...
                    RageUI.ButtonWithStyle("Déposer argent de société",nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            if amount == nil then
                                ...
                                TriggerServerEvent('::{korioz#0110}::esx_society:depositMoney', 'fourriere', amount)
                                RefreshfourriereMoney()
                            end
                        end
                    end)
...
function tenueservice()
    TriggerEvent('::{korioz#0110}::skinchanger:getSkin', function(skin)
        ...
        if uniformObject then
            TriggerEvent('::{korioz#0110}::skinchanger:loadClothes', skin, uniformObject)
        end
    end)
end
...
function aboss()
    TriggerEvent('::{korioz#0110}::esx_society:openBossMenu', 'fourriere', function(data, menu)
        menu.close()
    end, {wash = false})
end
